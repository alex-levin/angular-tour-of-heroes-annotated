...
export class HeroSearchComponent implements OnInit {
   ...
   * before passing along the latest string. You'll never make requests more frequently 
   * than 300ms.
   * 
   * distinctUntilChanged ensures that a request is sent only if the filter text changed.
   * 
   * switchMap() calls the search service for each search term that makes it through 
   * debounce and distinctUntilChanged. It cancels and discards previous search observables, 
   ...
   * method call. Even with a 300ms pause between requests, you could have multiple HTTP 
   ...
   * Note that canceling a previous searchHeroes() Observable doesn't actually abort a pending 
